...
[iOS `RouteCatalog`](https://github.com/Koshimizu-Takehito/my-toybox/blob/main/Packages/Sources/MyToyboxScreens/RouteCatalog.swift) の `availableRoutes` と **同じ `Screen.rawValue`** で、このリポジトリ直下に `<clip_screen_id>/index.html` を追加する（`layout: clip_landing`）。
